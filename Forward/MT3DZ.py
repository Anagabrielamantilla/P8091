...
def compute_mt_responses(mesh, sigma, receivers, frequencies, mu=None):
    ...
    # modificacion para identificar la topografía.
    air_threshold = 1e-6 # Este valor sirve cuando la conductividad del aire en el modelo es menor o igual a 1e-6 y el background y anomalias son mayores a 1e-6
    active_cells = sigma > air_threshold #se cambia below_surface por active_cells
    
    sigma_background = 2e-3 # El valor de background para las pruebas toy6
    #sigma_background = 1e-2 # El valor de background para las pruebas sin topografía 1D, 2D y 3D, siempre fue 1e-2  
    ...
    delta_sigma[active_cells] = sigma[active_cells] - sigma_background 
