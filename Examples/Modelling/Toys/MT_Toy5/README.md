# MT_Toy5 — Operador MT3DZ vs. SimPEG
...
El experimento **Toy5** corresponde a un modelo sintético tridimensional empleado para evaluar el comportamiento del operador frente a
variaciones espaciales de conductividad y validar el cálculo simultáneo del tensor de impedancias
y del tipper.
...
| Parámetro | Valor |
|---|---|
| Malla | `../../../../models/Toy5/mesh_Toy5_MT.json` — `TensorMesh` 14 × 14 × 22 (4 312 celdas) |
| Modelo de conductividad | `../../../../models/Toy5/model_Toy5_MT.npy` — σ en S/m, celdas de aire = 1e-8 |
...
1. **`MT_ModelToy5_MT3DZ.ipynb`** — modelamiento directo con el operador propio
   (`Forward/MT3DZ.py` → `compute_mt_responses`).
   Guarda `impedance_MT3DZ_Toy5.npy`, `tipper_MT3DZ_Toy5.npy` y la figura `MT3DZ_Toy5.png`.
2. **`MT_ModelToy5_SimPEG.ipynb`** — modelamiento directo con SimPEG y, al final, la
   **comparación** contra el operador. La sección de comparación no vuelve a correr ningún forward:
   ...
   MAE / RMSE / MAPE.
   Guarda `impedance_SimPEG_Toy5.npy`, `tipper_SimPEG_Toy5.npy` y las figuras `SimPEG_Toy5.png`
   y `MT3DZ_vs_SimPEG_Toy5.png`.

## Respuestas guardadas

| Archivo | Forma | Contenido |
|---|---|---|
| `impedance_MT3DZ_Toy5.npy` | `(n_rec, n_freq, 2, 2)` | tensor de impedancias del operador MT3DZ |
| `tipper_MT3DZ_Toy5.npy` | `(n_rec, n_freq, 2)` | tipper `[Tzx, Tzy]` del operador MT3DZ |
| `impedance_SimPEG_Toy5.npy` | `(n_rec, n_freq, 2, 2)` | tensor de impedancias de SimPEG |
| `tipper_SimPEG_Toy5.npy` | `(n_rec, n_freq, 2)` | tipper `[Tzx, Tzy]` de SimPEG |
...
| `SimPEG_Toy5.png` | resistividad aparente, fase y tipper de SimPEG |
| `MT3DZ_vs_SimPEG_Toy5.png` | las dos respuestas superpuestas + diferencia relativa |
...
- La diferencia crece hacia las **frecuencias bajas**. A 1e-4 Hz el *skin depth* supera con holgura
  el tamaño del dominio de `mesh_Toy5_MT.json`, así que la respuesta queda dominada por los bordes
